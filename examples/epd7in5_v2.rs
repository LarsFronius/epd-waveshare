...
use epd_waveshare::{buffer_len, graphics::VarDisplay};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let clock_buffer_width = (char_width * clock_string_length) as u32 + 2;
    let clock_buffer_height = char_height as u32 + 2;

    let mut buffer =
        vec![0x11; buffer_len(clock_buffer_width as usize, clock_buffer_height as usize)];
    let mut partial_display =
        VarDisplay::<Color>::new(clock_buffer_width, clock_buffer_height, &mut buffer, false)
            .unwrap();

    // Starting position for clock on screen
    let clock_start_x = 300u32;
    let clock_start_y = 200u32;
    ...
    for iteration in 0..10 {
        ...
        // Draw the entire time string on the clock buffer (with 1 pixel padding)
        Text::new(&current_time, Point::new(4, char_height + 1), text_style)
